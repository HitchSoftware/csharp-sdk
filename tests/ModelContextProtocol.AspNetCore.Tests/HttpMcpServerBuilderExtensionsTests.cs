...
public class HttpMcpServerBuilderExtensionsTests(ITestOutputHelper testOutputHelper) : KestrelInMemoryTest(testOutputHelper)
{
    ...
    public async Task IdleTrackingBackgroundService_StartsTimer_WhenStateful()
    {
        Builder.Services
            .AddMcpServer()
#pragma warning disable MCP9005 // Stateful Streamable HTTP option is obsolete but required to exercise the stateful timer path.
...
#pragma warning restore MCP9005
        ...
        // With Stateless = false the timer loop must start, so ExecuteTask should be set.
